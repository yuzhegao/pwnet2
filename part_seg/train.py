...
parser.add_argument('--model', default='pwnet2', help='Model name [default: model]')
...
os.system('cp ../models/pwnet2.py %s' % (LOG_DIR))
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    # {0:Airplane, 1:Airplane,   ... 47:Table, 48:Table, 49:Table}
    ...
    for batch_idx in range(num_batches):
        ...
        ## notice: the batch_data is fixed size- [B,N,6] =>so the batchsize is fixed, maybe benfit for batchnorm
        ...
        # Select valid data
        cur_pred_val = pred_val[0:cur_batch_size] ## [B,N,50]
        # Constrain pred to the groundtruth classes (selected by seg_classes[cat])
        cur_pred_val_logits = cur_pred_val  ## network pred
        cur_pred_val = np.zeros((cur_batch_size, NUM_POINT)).astype(np.int32) ## [B,N] all zeros

        for i in range(cur_batch_size): ## B
            cat = seg_label_to_cat[cur_batch_label[i,0]] ## get shape cls
            logits = cur_pred_val_logits[i,:,:] ## [N,50]
            cur_pred_val[i,:] = np.argmax( logits[:,seg_classes[cat]] , 1) \
                                                       + seg_classes[cat][0]
            ## [N,] + scalar
            ## just ignore the wrong part_id ???? , and get max-score id in correct part_ids


        correct = np.sum(cur_pred_val == cur_batch_label) ## correct num_pts
        total_correct += correct
        total_seen += (cur_batch_size*NUM_POINT) ## B*N
        if cur_batch_size==BATCH_SIZE:
            loss_sum += loss_val
        ...
        ## calculate part_iou for each shape
        ## shape_ious[cat] -> the list of part_iou  for all shape belongs to cat
        for i in range(cur_batch_size): ## B
            segp = cur_pred_val[i,:]
    ...
    mean_shape_ious = np.mean(shape_ious.values()) ## just get mean of all shapes in one cls, then get mean of all cls
